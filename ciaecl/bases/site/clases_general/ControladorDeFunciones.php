<?php
	...
	class Funciones 
	{
		function corregirCaracteres($contenido)
		{
			//$contenido = utf8_decode($contenido);

			$previo = array("Â³","Ã¡","Ã±","Ã³","Ã©","Ãƒ","Ã£","Â©","Â±","Â-","Â¡","\'","Ã¨","Ã¬","Ãº","Ã¼",'\"',"Âº","Â&deg;","Ã‘","Â¿","Ã-","Â-");
			$posterior = array("&oacute;","&Aacute;","&ntilde;","&oacute;","&eacute;","","","&eacute;","&ntilde;","&iacute;","&Aacute;","'","&eacute;","&iacute;","&uacute;","ü",'"',"&uacute;","º","&ntilde;","¿","&iacute;","&iacute;");
			$contenido = str_replace($previo,$posterior,$contenido);
			$contenido = str_replace($previo,$posterior,$contenido);
			$contenido = str_replace("Ã","&iacute;",$contenido);

			return $contenido;
		}

		function generarTabla($output,$con_fila=true,$con_strip=true)
		{
			$total = count($output);
			$contenido = ''; 
		...
		{ 
			Funciones::mostrarAlertaSimple('No tiene permiso para acceder a este menu');  
		}
		
		
		function mostrarAlertaSimple($texto)
		{
			echo "<script>alert('".$texto."');</script>";  
		}

		function sesionCaducada($msg='Su sesi&oacute;n ha finalizado')
		{
			global $ControlHtml; 
...
'),array(" "," "," "," "," "),$texto);*/ 
		...
		function sendEmail($usuario,$asunto='',$mensaje,$bcc_send=false,$es_masivo=false,$bcc_extra='',$replay_to='',$remitente='webmaster')
		{		
		
			echo $usuario." ".$asunto." TEST";
			...
			if(!VarConfig::estadoDebug)
			{
				...
				//Funciones::sendEmailLogin( $para, $asunto, $mensaje);
				switch($remitente)
				{
					case 'informaciones':
						Funciones::sendEmailLoginGmail( $para, $asunto, $mensaje,VarConfig::site_email_send_info,VarConfig::site_email_send_pass_info);
					break;
					case 'bh':
						Funciones::sendEmailLoginGmail( $para, $asunto, $mensaje,VarConfig::site_email_send_bh,VarConfig::site_email_send_pass_bh);
					break;
					default:
						Funciones::sendEmailLoginGmail( $para, $asunto, $mensaje,VarConfig::site_email_send_webmaster,VarConfig::site_email_send_pass_info);
					break;
				} 
				/* SOLO SE ENVIAR&Aacute; SI ES EL SITIO DE PRODUCCION*/
			}	
			else
			{
				//Funciones::sendEmailLogin( $para, $asunto, $mensaje);
		...
		function sendEmailLoginGmail( $para, $asunto, $mensaje,$usuario,$password,$bcc='')
		{  
			...
			$phpmailer->Host       = VarConfig::site_email_send_server_gmail; // SMTP server
			$phpmailer->Port       = VarConfig::site_email_send_port_gmail;    // set the SMTP port for the GMAIL server; 465 for ssl and 587 for tls
			$phpmailer->SMTPSecure = 'tls'; 
			$phpmailer->Username   = $usuario; // Gmail account username
			$phpmailer->Password   = $password;        // Gmail account password
			$phpmailer->AddReplyTo($usuario,   VarConfig::site_title);
			if(trim($bcc) != '')
			{
			...
			$phpmailer->SetFrom($usuario, VarConfig::site_title);  
			...
			if(!$phpmailer->Send()) 
			{
				...
				return Funciones::sendEmailLogin( $para, $asunto, $mensaje);
				//return false;
 			} 
			 else 
			{
		...
		function sendEmailLogin( $para, $asunto, $mensaje)
		{  
			...
			if(!$phpmailer->Send()) 
			{ 
				//echo "<!-- Mailer Error 2: " . $phpmailer->ErrorInfo.' -->';
				return false;
			} 
			else 
			{
		...
		function  cleanSqlInjection($str,$extra_log = false) 
		{	  
			$str = addslashes($str);
			...
			{
				$bad = array(
							'sleep'				=> '[removed_SLEEP_removed]',  
							'benchmark'			=> '[removed_BENCHMARK_removed]',
							'union'				=> '[removed_UNION_removed]',
							'name_const'		=> '[removed_NAME_CONST_removed]',							
							'drop'				=> '[removed_DROP_removed]',
							'select'			=> '[removed_SELECT_removed]', 
							'from'				=> '[removed_FROM_removed]',
							'where'				=> '[removed_WHERE_removed]',
							'delete'			=> '[removed_DELETE_removed]'
						);
				$str = strtolower($str);
		...
		{			
			...
			if(trim($aux) == trim($sql)) 
			{
				...
				{ 
					case 'sql-update':
						$aux = str_replace('UPDATE common_','',trim($sql));
					break;
					case 'sql-delete':
						$aux = str_replace('DELETE FROM common_','',trim($sql));
					break;   
					default:
						$aux = str_replace('INTO common_','',trim($sql)); 
					break;
				}  
				if(trim($aux) != trim($sql))
				{
		...
		function mostrarArregloProduccion($arreglo,$titulo='')
		{
			echo "<!-- INICIO DEBUG EN PRODUCCION ".strtoupper($titulo)."---\n";
			print_r($arreglo);
			echo "<!-- FIN DEBUG EN PRODUCCION ".strtoupper($titulo)."--->\n";
		}
		...
		{	 	
			...
			else
			{
				...
				if(VarConfig::estadoDebugPublic)
				{
					$indexOutput .= "\n<!-- ".$salida." -->\n";
				}				
				else
		...
		/* PERMITE IDENTIFICAR QUE TIPO DE DISPONSITIVO VISITA EL SITIO, PARA PODER MOSTRAR LOS TEMPLATES ASOCIADOS 
		casos que despliega
		mobile
		tablet
		desktop
...
//			$body_class = 'desktop';
	...
	/************************************************************************************************
					CLASE DE GESTI&oacute;N DE FECHAS Y HORAS
	************************************************************************************************/
	class ControladorFechas 
	{		
		/* FUNCIONES DE ADMINISTRACION DE FECHAS */
		function formatoFechaSql($simple=false)
		{			
			$output = '%d-%m-%Y';
			if(!$simple)
			{				
		...
		function entregarMesAbr($fecha,$lang='es')
		{
			$Mes=explode(" ", $fecha);
			$lang='es';
			if($lang == 'es')
			{
				if ($fecha=='January') $Mes='Ene';
				if ($fecha=='February') $Mes='Feb';
				if ($fecha=='March') $Mes='Mar';
				if ($fecha=='April') $Mes='Abr';
				if ($fecha=='May') $Mes='May';
				if ($fecha=='June') $Mes='Jun';
				if ($fecha=='July') $Mes='Jul';
				if ($fecha=='August') $Mes='Ago';
				if ($fecha=='September') $Mes='Sep';
				if ($fecha=='October') $Mes='Oct';
				if ($fecha=='November') $Mes='Nov';
				if ($fecha=='December') $Mes='Dic';
			}
			else
			{
				$Mes=$Mes;
		...
		function transformarFechaHTML($texto,$key_original='fecha',$key_nuevo='fecha_html_full')
		{ 
			$total = count($texto);
			for($i=0; $i < $total; $i++)
			{ 
				$texto[$i][$key_nuevo] = ControladorFechas::invertirFecha($texto[$i][$key_original]); 				 
			} 
			return $texto;
		}  
		
		function fechaTexto($fecha,$traduccion='en-es',$minuscula=false)
		{
			$time = ControladorFechas::fecha2Time($fecha);
			$fecha_texto = ControladorFechas::traducirMes(date("d \d\e F \d\e Y",$time),$traduccion,$minuscula);
			return $fecha_texto;
		}
		
		
		function traducirMes($texto,$traduccion='en-es',$minuscula=false)
		{ 
			if(VarSystem::obtenerIdiomaActual() == 'en')
			{
				return $texto;
			}
			Funciones::mostrarArreglo(array($texto,$minuscula),false);
			$espanol = array('Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre');
			$ingles = array('January','February','March','April','May','June','July','August', 'September', 'October', 'November', 'December');  
			...
			Funciones::mostrarArreglo($texto,false);
			return $texto;
		}
	}
?>