...
{	
	function guardarArchivo($datos)
	{
		/******************************************************
	            ...
	            [path]   
		********************************************************/
		$datos['extension'] = substr($datos['name'],-3);
		$datos['md5'] = md5(time().$datos['name']);
		if ( ! @copy($datos['tmp_name'],$datos['path'].$datos['md5'].'.'.$datos['extension']))
		{
	...
	{		
		...
		SIDTOOLHtml::escribirArchivo($nombre_archivo,$contenido);
		return $file;
	}
	...
	function limpiarArchivosTemporales($path)
	{
		...
		$limite   	= 432000; /* segundos => 5 d&iacute;as*/
	...
	{
		return mb_convert_case(ucwords(strtolower(trim($text))), MB_CASE_TITLE);
	} 

	function cleanHtml($input,$decode=false) 
	{  
		return Funciones::cleanHtml($input,$decode);
	}
...
class SIDTOOLPost
{
	...
	{	 
		return Funciones::cleanSqlInjection($str);;
	} 
}
