 ...
 *
 * @package ciae_web
 * @author Paulina Sep&uacute;lveda psepulve@gmail.com :: CIAE - Uchile
 * @copyright 2013
 * @version $Id$
 * @access public
 */
	...
	{ 
		...
		else
		{
			...
			else
			{
				...
				{
					/** SE BUSCA REVISION NUEVA Y SE LE AGREGA EL D&iacute;A DE HOY */
...
class ControlGeneralCorreccion
{
	...
	function asignacionDefinitiva($id_texto,$valores)
	{
		...
		if($CorreccionDefinitivo->estado == 'pendiente') /* PARA SOLO CAMBIAR AQUELLOS QUE EST&eacute;N PENDIENTES */
		{  
...
//		Funciones::mostrarArreglo($valores,true);  
		...
		if(is_array($listado) && $total > 0)
		{
			switch($total)
			{
				...
				default:
					...
					{ 
						/** QUIERE DECIR QUE EST&Aacute;N TODOS LOS TEXTOS CERRADOS Y HABR&iacute;A QUE REVISAR SI HAY INCONSISTENCIAS */ 
						...
						{
							/* NO HAY INCONSISTENCIAS Y TODAS LA REVISIONES FUERON IGUALES Y HABR&iacute;A QUE PASARLO A DEFINITIVO Y CERRARLO */
	...
	function revisionCierreProceso($valores)
	{
		...
		{
			/** SE CIERRA EL PROCESO DEL DIA SI YA SE COMPLETO LA REVISI&oacute;N */
