<?php 
	...
	{ 
		...
		function generarArchivoReporteExcell($contenido,$nombre_archivo='',$archivo_reporte='')
		{ 
			$path 		= VarSystem::getPathVariables('dir_repositorio_tmp');
			if(trim($archivo_reporte) == '')
			{
				$contenido	= Funciones::generarTabla($contenido);
			}
			else
			{
				$MantenedoresGeneral = new MantenedoresGeneral(); 
				$template_reporte = new miniTemplate($archivo_reporte);
				$template_reporte = $MantenedoresGeneral->mostrarListadoGenerico($template_reporte,$contenido,'lista_item');
				$contenido = $template_reporte->toHtml();
			}
			$archivo 	= SIDTOOLHtml::escribirExcelTabla($contenido,$path,'xls',$nombre_archivo); 
		...
		function showVolverOpcionSimple($opcion='home')
		{
			return $this->showVolver($opcion);
		}
...
"," ",$output);*/
		...
		{
			//Funciones::mostrarArreglo(array($pagina,$tamPag,$total),true,'antes paginamientoLimite');
			if(trim($pagina) == '')
			{
				$pagina = 1;
			}
			...
			//Funciones::mostrarArreglo(array($limite,$numPags,$limitInf),true,'despues paginamientoLimite');
			return array($limite,$numPags,$limitInf);
		}
		...
		function despliegueListadoReportes($reportes)
		{
			return $this->despliegueListadoGenericos($reportes,'reportes');
		}
		
		function despliegueListadoManuales($manuales)
		{
			return $this->despliegueListadoGenericos($manuales,'manuales');
		}
		
		function despliegueListadoGenericos($arreglo,$caso)
		{
		    $e 	= new miniTemplate(VarSystem::getPathVariables('dir_template_sitio').'admin/descarga_'.$caso.'_estandar.tpl');
			if(is_array($arreglo) && count($arreglo) >0)
			{				 
		    	$e->addTemplate('lista_'.$caso.'_descarga_estandar');
          		$MantenedoresGeneral = new MantenedoresGeneral(); 
		    	$e = $MantenedoresGeneral->mostrarListadoGenerico($e,$arreglo,'lista_'.$caso.'_descarga_estandar_elemento'); 
			}
			return $e->toHtml();
		}
		
		function paginamientoListado($pagina,  $inicio, $final, $numPags, $recurso, $total='')
		{ 
			//Funciones::mostrarArreglo(array($pagina,  $inicio, $final, $numPags),true,'antes paginamientoListado');
			/*
				$pagina => p&Aacute;gina seleccionada 
				$inicio => p&Aacute;gina inicial que se mostrar&Aacute; en vista html
				$final => p&Aacute;gina final que se mostrar&Aacute; en vista html
				$numPags => total de p&Aacute;ginas que se mostrar&Aacute;n
			*/
		 	//Funciones::mostrarArreglo(array($pagina,  $inicio, $final, $numPags),true);
		    if ($recurso=='recurso'){
			$e 	= new miniTemplate(VarSystem::getPathVariables('dir_template_general').'paginamiento_recursos.tpl');
			}else{
		    $e 	= new miniTemplate(VarSystem::getPathVariables('dir_template_general').'paginamiento.tpl');
			}
			if(trim($total) != '')
			{
				$e->addTemplate('bloque_paginamiento_total');
