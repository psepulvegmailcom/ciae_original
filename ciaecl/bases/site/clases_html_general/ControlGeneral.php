<?php
	...
	class ControlGeneral
	{
		...
		function mostrarObjetoListado($e = '')
		{
			...
			$e = $this->MantenedoresGeneral->mostrarListado(
				$e,
				$listado,
				$this->ControlObjeto->obj->dbKey,
				$this->opcion
			);
			return $e;
		}
		...
		function mostrarListado($e = '')
		{
		    ...
		    $this->ControlHtml->elUsuarioSitios;
			$this->Sitio = $this->ControlHtml->elUsuarioSitios;
			$this->SitioUser = $this->Sitio['0']['id_site'];
            if ($this->SitioUser > 1) {
            				$this->ControlClase->where = 'id_site='.$this->SitioUser;
			...
			$e->setVariable('opcion_modulo',$this->lastAction[0]);
		    $e = $this->MantenedoresGeneral->mostrarSeleccion(
				$e,
				'bloque_form_id_orden',
				$arregloCampos,
				'',
				'id_orden'
			);
            ...
            if(trim($this->valores['buscar_filtro_busqueda']) != '')
            {
            	            if ($this->SitioUser > 1) {
            				$this->ControlClase->where = 'id_site='.$this->SitioUser;
							$this->ControlClase->where = $this->ControlClase->where." AND ";
							}
			...
			list(
				...
				$tamPag,
				$limitInf
			) = $this->MantenedoresGeneral->calculoPaginasPaginamiento(
				$total,
				$this->valores
			);
			if ($this->ControlClase->sourceTable == "view_publicaciones_tipo")
			{
				$limite = "";
			}
			...
			else
			{
				...
				$paginamiento = $this->FormGeneral->paginamientoListado(
					$pagina,
					$inicio,
					$final,
					$numPags,
				...
				$e = $this->MantenedoresGeneral->mostrarListado(
					$e,
					$ListaDeObjetos,
					$this->ObjetoClase->dbKey,
					$this->lastAction[0],
					$limitInf
				);
			}
       		$e = $this->MantenedoresGeneral->mostrarSeleccionOrdenSimple(
				$e,
				'bloque_form_id_orden',
				$this->arregloCamposOrdenar
			);
			return $e;
		}
	}
...
class MantenedoresGeneralObjeto
{
   ...
   */
	...
	{
		$ObjetoClase = $this->setearVariablesGuardarObjetoSimple(
			$ObjetoClase,
			$valores,
			$prefijo
		);
   ...
   */
	...
	function calculoPaginasPaginamiento($total, $valores)
	{
		...
		list(
			...
			$numPags,
			$limitInf
		) =  $FormGeneral->paginamientoLimite(
			$pagina,
			$tamPag,
			$total
		);
	...
	)
	{
		list(
			...
			$strQry,
			$order
		) = parent::obtenerQryPorBusqueda(
			$valores,
			$tipoBusqueda,
			$valoresBusqueda,
			$valBusquedaPalabra
		);
		$listadoN = $ControlObjeto->obtenerListadoPorBusqueda(
			$strQryRes,
			$strQry,
			$order
		);
		...
		list(
			...
			$tamPag,
			$limitInf
		) = $this->calculoPaginasPaginamiento($total, $valores);
		$listado = $ControlObjeto->obtenerListadoPorBusqueda(
			$strQryRes,
			$strQry,
			$order,
			$limite
		);
		 list($inicio,$final) = $this->calculoPaginasPaginamientoAjuste(
			$pagina,
			$tamPag,
			$numPags
		);
	...
	{
		list($strQry, $strQryRes) = parent::obtenerLeyendaPalabraBusqueda(
			$valores,
			$tipoBusqueda,
			$valBusquedaPalabra
		);
