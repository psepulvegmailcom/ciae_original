<a name="formulario"></a>
 <div class="contenido_titulo_separador">	 Presentaci&oacute;n de investigaci&oacute;n "Estudios de producci&oacute;n escrita en escolares"  <br> "Caracter&iacute;sticas de la producci&oacute;n escrita de ni&ntilde;os de 3&deg; a 8&deg; b&aacute;sico"
</div>

 <input type="hidden" name="form_tipo_inscripcion" value="20120529_produccionescrita">
 
<table class="tabla_simple" border="0">
 
<tr>
<td style="width:60% ">
<div id='inscripcion_ficha'> 
 <!-- INCLUDE BLOCK : www/templates/web/inscripcion/formulario_mensaje.tpls -->
 
 
 
 <!-- INCLUDE BLOCK : www/templates/web/inscripcion/formulario_simple.tpls -->
 <!-- INCLUDE BLOCK : www/templates/web/inscripcion/cerradas.tpl -->
 
 
 </div>
 
 
 
  
 
  
</td>
<td>
 
<!-- INCLUDE BLOCK : www/templates/web/bloque_redondeado_header.tpl --> 
 <li  class="lateral_inscripcion" style="font-size:13px; " ><strong>Fecha:</strong> <strong>29 de mayo de 2012</strong></li> 
 <li  class="lateral_inscripcion"style="font-size:13px; "  ><strong>Hora:</strong> 17:00 - 19:00 hrs  </li>
  
 <li class="lateral_inscripcion" style="font-size:13px; " > <strong>Lugar:</strong> Casa Amanda Labarca<br /> Centro de Investigaci&oacute;n Avanzada en Educaci&oacute;n <br /> Universidad de Chile<br />Calle Periodista Jos&eacute; Carrasco Tapia (Ex Belgrado) nº 75.<br />Metro Baquedano<br />
Santiago, Chile
  </li>  
 <li class="lateral_inscripcion" style="font-size:13px; " ><strong>Contacto: </strong>
<a href="mailto:ciae@uchile.cl">ciae@uchile.cl</a> &oacute; 978 9723<br /> 
</li> 
<!-- INCLUDE BLOCK : www/templates/web/bloque_redondeado_footer.tpl --> 
 <br />
 <p> 
 </p>
</td>
 </tr> 
</table>
<script type="text/javascript">
function ocultarDiv()
{
	hiddenId('inscripcion_ficha');   
}

function mostrarDiv(id)
{ 
	ocultarDiv();
	showId(id);
}
ocultarDiv();
mostrarDiv('inscripcion_ficha');

</script>