...
//--> begin Time Clock addition	
	...
	function pGetHHvalue() {
	
		if($this->HHvalue > 0 )
			return "$&nbsp;".number_format($this->HHvalue);
		else
			return '-';
	}
	
	function pGetProjectValue($type='real') {
	
		$objItemList = new ItemStats();
		
		$sqlFilter = 'ii.projectId = \''.$this->id.'\'';
		...
		if ($objItemList->rMore()) {
			...
			while ($objItem = $objItemList->rNext()) {
				$cost = $cost + $objItem->getItemCost($type,false);
				//echo $type." ".$objItem->getItemCost($type)."<br>"; 
			}	
	...
	function pGetEstimateTime($projectId) {
		// get total time
        $sql = 'SELECT SUM(expectedDuration) as itemTotalTime '
	   		.'FROM '.$this->gTable('item').' as it '
			.'WHERE it.projectId = '.$projectId.' LIMIT 1';
		$this->getConnection();
		$totalTime = 0;
		if ($result = $this->query($sql)) {		
			if ($data = $result->rNext()) {				
				$totalTime = $data->itemTotalTime;
			}  		
        }
		if($totalTime == 0)
			$totalTime = '-';
