		...
		<table cellpadding="2" cellspacing="0" border="0" align="center" width="95%">
    	  <tr>
    	    ...
    	    <td><?php
                ...
                $objTemp->qSelect('priority',($objTask->priority)?$objTask->priority:1,'','','style="width:40px"');
            ?></td>
		<?php
			if (@constant('FRK_CONTEXT_ENABLE')) {
		?>
			...
			<td><?php
                $objContextList->qSelect('context',$objTask->context,'','width:130px');
            ?></td>
		<?php
			}
		?>
    	  ...
    	  <tr valign="top">
			...
			<td<?php echo (@constant('FRK_CONTEXT_ENABLE'))?' colspan="3"':''; ?>>
			<textarea name="description" class="full"  style="width:100%" ><?php echo $objTask->description; ?></textarea></td>
    	  </tr>
		  <?php } ?>
          <tr>
		  ...
		  <tr><td>HH Estimadas</td><td><input type="text" name="expectedDuration" value="<?php  if($objTask->expectedDuration > 0) echo $objTask->expectedDuration; else echo ''; ?>"  class="dead" <?php if($objUser->level <= 2) echo "readonly=\"readonly\"";?> /></td></tr>
		   <tr><td>Urgencia</td><td><?
		   $objUrgencyList = new ItemUrgencyList();
		   $objUrgencyList->qSelect('expectedUrgency',$objTask->expectedUrgency,'','width:150px');                         
		   ?>
		   &nbsp;&nbsp;&nbsp;&nbsp;Complejidad</td><td><?
		   $objComplexList = new ItemComplexList();
		   $objComplexList->qSelect('expectedComplex',$objTask->expectedComplex,'','width:150px');                         
		   ?></td>
		   </tr>
    	  <tr>
