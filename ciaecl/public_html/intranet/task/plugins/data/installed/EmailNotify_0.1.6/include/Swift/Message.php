...
if (!defined("SWIFT_VERSION")) define("SWIFT_VERSION", "3.1.5_4");
...
class Swift_Message extends Swift_Message_Mime
{
  ...
  function generateId($idstring=null)
  {
    $midparams =  array(
      ...
      "pid" => getmypid(),
      "randint" => mt_rand(),
      "customstr" => (preg_match("/^[A-Za-z0-9]\$/D", $idstring) ? $idstring : "swift") ,
      "hostname" => (isset($_SERVER["SERVER_NAME"]) ? $_SERVER["SERVER_NAME"] : php_uname("n")),
    );
    $this->setId(vsprintf("<%ld.%d.%d.%s@%s>", $midparams));
    return $this->getId();
  }
  /**
  ...
  function setPriority($priority)
  {
    ...
    $this->headers->set("X-MimeOLE", "Produced by SwiftMailer " . SWIFT_VERSION);
  }
  /**
