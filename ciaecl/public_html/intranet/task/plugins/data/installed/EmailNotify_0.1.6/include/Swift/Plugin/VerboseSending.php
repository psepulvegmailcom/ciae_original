...
class Swift_Plugin_VerboseSending extends Swift_Events_Listener
{
  ...
  function sendPerformed(&$e)
  {
    ...
    foreach ($recipients->getTo() as $address => $o)
    {
      $pass = !in_array($address, $failed);
      $view =& $this->getView();
      $view->paintResult($address, $pass);
    }
    foreach ($recipients->getCc() as $address => $o)
    {
      $pass = !in_array($address, $failed);
      $view =& $this->getView();
      $view->paintResult($address, $pass);
    }
    foreach ($recipients->getBcc() as $address => $o)
    {
      $pass = !in_array($address, $failed);
      $view =& $this->getView();
      $view->paintResult($address, $pass);
