...
function fValidaEditArchivo(form, msg)
{
	  if(hay_responsables)
	  {
		  	var seleccion_usuarios = false;
		  	var x = document.feditaarchivo.elements;	
			for (var i=0 ; i < x.length ; i++)
			{			    
				if(x[i].name == 'usuarios[]' && x[i].checked)
				{			
					 seleccion_usuarios = true;								
				}
			}
			if(! seleccion_usuarios)
			{
				  alert('No ha seleccionado ningun usuario responsable de este documento. Por favor ingrese toda la información.');	
				  return false;
			}
	  }
	if(vacio(form.titulo.value)) 
	{
	 ...
	 /* if(vacio(form.pais.value)) {
	  ...
	  if(vacio(form.archivo_anterior.value) && vacio(form.archivo.value))
		{
		  alert('No hay ingresado el archivo adjunto 1. Por favor ingrese toda la información.');	
		  return false;
	  }
	  if(vacio(form.archivo_anterior_2.value) && vacio(form.archivo_2.value))
		{
		  alert('No hay ingresado el archivo adjunto 2. Por favor ingrese toda la información.');	
		  return false;
	  }*/
